...
* Added checkpointed, idempotent UTC daily aggregates and resumable day/range rebuilds.
* Added non-overlapping raw/aggregate reporting with mergeable visitor/session estimates and additive engagement formulas.
* Cleanup now requires verified aggregate coverage before deleting raw history.
* Added two aggregate tables and bounded high-cardinality dimensions; existing analytics and metadata remain intact.

= 0.7.0 =

* Added chunked CSV/JSON exports with privacy-preserving IP modes and current analytics, metadata, and exclusion filters.
* Added unlimited/preset/custom retention, cleanup preview, bounded resumable WP-Cron cleanup, locking, and observable state.
* Added storage and monthly-count diagnostics without changing the v0.6.3 schema.
...
Adds two daily-aggregate tables and preserves supported long-range statistics before raw cleanup. Back up WordPress and allow aggregation to complete before enabling finite retention.

= 0.7.0 =

Adds export, retention, cleanup, and storage diagnostics. No schema migration is required; review retention before deleting raw history.
