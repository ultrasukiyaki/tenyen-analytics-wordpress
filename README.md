...
Upload the release ZIP in **Plugins → Add New → Upload Plugin**, or copy the `tenyen-analytics` directory to `wp-content/plugins/`, then activate it. Existing v0.5.2 installations can be updated by overwriting the plugin directory; v0.5.5 requires no database migration.
...
Back up WordPress, then overwrite or upload the new plugin version. Keep GeoLite2 files in the uploads directory. Version 0.5.5 retains existing tables, options, routes, payload fields, preferences, and collected data.
