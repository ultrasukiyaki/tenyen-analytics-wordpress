(() => {
  ...
  root.querySelector('[data-cleanup-preview]').addEventListener('click',async()=>{cleanupResult.textContent=__('Loading…','tenyen-analytics');try{const p=(await request(config.preview,{method:'POST',body:'{}'})).preview;cleanupResult.textContent=`${__('Cutoff','tenyen-analytics')}: ${p.cutoff||'—'}\n${__('Affected events','tenyen-analytics')}: ${p.events}\n${__('Affected sessions','tenyen-analytics')}: ${p.sessions}`;}catch(error){cleanupResult.textContent=error.message;}});
